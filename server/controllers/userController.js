...
export const getUserData = async (req, res) => {
    try {
        const { userId } = req.auth; // Remove await
...
export const updateUserData = async (req, res) => {
    try {
        ...
        let finalUsername = username || tempUser.username;

        if (tempUser.username !== finalUsername) {
            const user = await User.findOne({ username: finalUsername });
            if (user) {
                // we will not change the username if it is already taken
                finalUsername = tempUser.username;
            }
        }
        ...
        if (cover) {
            ...
            const response = await imagekit.upload({
                file: buffer,
                fileName: cover.originalname, // Fixed: was profile.originalname
            });
